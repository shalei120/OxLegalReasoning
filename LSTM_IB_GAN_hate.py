...
class LSTM_IB_GAN_Model(nn.Module):
    ...
    def build(self, x, eps=0.000001):
        ...
        s_w_feature_norational = self.z_to_fea(en_outputs_norational)
        z_nero_norational, _ = torch.max(s_w_feature_norational, dim=1)  # batch hid
        output_norational = self.ChargeClassifier(z_nero_norational).to(args['device'])  # batch chargenum
    ...
    def predict(self, x):
        ...
        return torch.exp(output), (torch.argmax(output, dim=-1), sampled_words, wordsamplerate, prob_rational, torch.exp(output_all), torch.exp(output_norational))
...
def test(textData, model, datasetname, max_accuracy, eps = 1e-6):
    ...
    with torch.no_grad():
        ...
        for batch in textData.getBatches(datasetname):
            ...
            answer = F.one_hot(torch.LongTensor(answer), num_classes=3)  # batch c
            ...
            pred_rationales = sampled_words[:,:,1].unsqueeze(1) # b 1 len
            ...
            for gold_rationale, p_ret in zip(batch.rationals, prob_rational):  # 3 len    // len
                largest_auc = 0
                for i in range(3):
                    p_axis, r_axis, thres = precision_recall_curve(gold_rationale[i,:], p_ret)
            ...
            comprehensiveness += ((output_all - output_norational) * y_onehot_T.float()).sum()
    ...
    res['auroc'] = roc_auc_score(total_y_true, total_y_score)


    res['ioup'] = (IOU_F1_correct_num / (IOU_F1_pred_num + 1e-6)).numpy()
    res['iour'] = (IOU_F1_correct_num / (IOU_F1_gold_num + 1e-6)).numpy()
    res['iouf1'] = 2 * res['ioup'] * res['iour'] / (res['ioup'] + res['iour'] + 1e-6)
    res['tokenF1'] = token_f1_sum / total
