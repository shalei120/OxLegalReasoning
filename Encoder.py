...
class Encoder(nn.Module):
    def __init__(self, args,w2i, i2w, embedding, device):
        """
        Args:
        ...
        if args['encunit'] == 'lstm':
            self.enc_unit = nn.LSTM(input_size=args['embeddingSize'], hidden_size=args['hiddenSize'],
                                    num_layers=args['enc_numlayer']).to(device)
    ...
    def forward(self, encoderInputs, encoder_lengths):
        ...
        # print(x['enc_input'])
        self.encoderInputs = encoderInputs.to(self.device)
        self.encoder_lengths = encoder_lengths

        self.batch_size = self.encoderInputs.size()[0]
        self.enc_len = self.encoderInputs.size()[1]

        enc_input_embed = self.embedding(self.encoderInputs).to(self.device)#.cuda()   # batch enc_len embedsize  ; already sorted in a decreasing order
        # dec_target_embed = self.embedding(self.decoderTargets).cuda()   # batch dec_len embedsize

        en_outputs, en_state = self.encode(enc_input_embed, self.encoder_lengths, self.batch_size) # seq batch emb
    ...
    def encode(self, inputs, input_len, batch_size):
        inputs = torch.transpose(inputs, 0, 1)
        hidden = (
        autograd.Variable(torch.randn(args['enc_numlayer'], batch_size, args['hiddenSize'])).to(self.device),
        autograd.Variable(torch.randn(args['enc_numlayer'], batch_size, args['hiddenSize'])).to(self.device))
