...
class LSTM_IB_GAN_Model(nn.Module):
    ...
    def __init__(self, w2i, i2w):
        ...
        self.encoder_select = Encoder(w2i, i2w, self.embedding).to(args['device'])
    ...
    def build(self, x, eps = 0.000001):
        ...
        z_prob = self.x_2_prob_z(en_outputs_select.to(args['device'])) # batch seq 2
...
def train(textData, model_path = args['rootDir'] + '/chargemodel_LSTM_IB_GAN.mdl', print_every=10000, plot_every=10, learning_rate=0.001, n_critic = 5):
    start = time.time()
    plot_losses = []
